...
# It uses saturated groundwater flow simulations and incorporates conditional probability distributions
# as petrophysical relationships, including a poorly calibrated Archie's law and an uncalibrated CR–k (CK) relationship.
# Additionally, this class allows the inclusion of water table observations to constrain the HBI inversion.
# For this, the groundwater probability is weighted by a multivariate Gaussian distribution,
# centered at the observed Zwt values with corresponding standard deviations.
# Water level observations and their uncertainties are read from the file 'Zwt_observations.txt'.
    ...
    def __init__(self,model_lst,setup_file,seed=0,params_hat=None):
        ...
        #Calculate possible sets for Zwt
        Z=[]
        Nz=int((self.Zmax-self.Zmin)/self.dZ)
        for i in np.arange(Nz):
             Z.append([self.Zmin+self.dZ*i,self.Zmax])
        self.Nz=len(Z)
        self.Z=np.array(Z)
        ...
        self.read_Zwt_observations(seed)
        self.proposal_pdf_list.append(stats.uniform(0,self.Nz)) 
    ...
    def get_param_proposal(self,modelbk_lst):
        ...
        params[1]=np.min((modelbk_lst[0].mesh.Y))
    ...
    def get_wt_level(self,model,params):
        ...
        Zwt=self.WTmodeling(K,Zwtx)

        self.prior_logpdf=self.Zwt_obs_loglike.logpdf(Zwt[self.nx_Zwt_obs])
        self.Zwt_mean=Zwt[self.nx_Zwt_obs]
        return Zwt
    ...
    def get_prior_logpdf(self,params,model_lst=[]):
        ...
        logpdf+=self.target_pdf_list[-2].logpdf(params[-2])
        logpdf+=self.target_pdf_list[-1].logpdf(params[-1])
        logpdf+=self.prior_logpdf#*10#*10#100    
    ...
    def read_Zwt_observations(self,seed):
        Zwt_obs=np.loadtxt('Zwt_observations.txt')
        X=self.model.mesh.X
        self.nx_Zwt_obs=[]
        self.Zwt_obs_hat=[]
        self.Zwt_obs_var=[]
        
        for i in np.arange(len(Zwt_obs)):            
            Xobs=Zwt_obs[i,0]
            Zwt_obs_hat=Zwt_obs[i,1]
            Zwt_obs_var=Zwt_obs[i,2]**2
            self.nx_Zwt_obs.append(np.argmin((Xobs-self.model.mesh.X)**2))
            self.Zwt_obs_hat.append(Zwt_obs_hat)
            self.Zwt_obs_var.append(Zwt_obs_var)
 
        self.Zwt_obs_loglike=stats.multivariate_normal(mean=np.array(self.Zwt_obs_hat), cov=np.diag(np.array(self.Zwt_obs_var)),seed=seed)
